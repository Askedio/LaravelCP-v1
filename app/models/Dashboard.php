...
class Dashboard extends Eloquent {
	...
	static public function activity(){
		return Activity::whereRaw('UNIX_TIMESTAMP(`activity_log`.`created_at`) > ? AND (activity_log.content_type="notification" OR activity_log.content_type="login")',array(Session::get('usersonline_lastcheck', time())))
							->select('description', 'details', 'users.displayname', 'content_type')
							->groupBy('description', 'details', 'users.displayname', 'content_type')
	...
	static public function online($value=10){
		return DB::table('users')->select('email', 'displayname', 'id', 'last_activity')->where(DB::raw('UNIX_TIMESTAMP(last_activity)'), '>', time()-150)->take($value)->get();
	}

	static public function onlineTotal(){
		$total=DB::table('users')->select(DB::raw('count(*) as total'))->where(DB::raw('UNIX_TIMESTAMP(last_activity)'), '>', time()-150)->get();
